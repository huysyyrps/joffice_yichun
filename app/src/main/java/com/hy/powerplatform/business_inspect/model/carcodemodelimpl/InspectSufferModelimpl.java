...
public class InspectSufferModelimpl implements InspectSufferModel {

    @Override
    public void getInspectSufferModelData(String httpTag, Context context, final BaseModeBackLisenter baseModeBackLisenter) {
        HttpUtils.initlist(HttpUtils.getService(RetrofitService.class).inspectSuffer()
                ,new MySubscriberbean(httpTag,context, Constant.GETDATA, new BaseRequestBackLisenter<InspectSuffeer>() {
                    @Override
                    public void success(InspectSuffeer inspectSuffeer) {
                        baseModeBackLisenter.success(inspectSuffeer);
                    }
