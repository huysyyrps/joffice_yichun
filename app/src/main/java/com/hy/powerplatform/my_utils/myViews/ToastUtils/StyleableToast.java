...
import static com.hy.powerplatform.my_utils.myViews.ToastUtils.Utils.getTypedValueInDP;
...
public class StyleableToast implements OnToastFinished {
    ...
    //这个构造方法可以设置使用自定义时间
    public StyleableToast(Context context, String toastMsg, boolean customTime, int duration) {
        this.context = context.getApplicationContext();
        this.toastMsg = toastMsg;
    ...
    public void show() {
        ...
        //使用自定义Toast来，控制显示时间，否则使用系统自带的Toast
        if(isCustomTime){
            //防止多次添加view到Window
            if(toastCustom != null){
                toastCustom.cancel();
            }
            toastCustom  = ToastCustom.makeText(context.getApplicationContext(),getToastLayout(), duration);
            toastCustom.show();
        ...
        //设置toast消失时的回调
        if (isAnimation) {
            durationTracker = new ToastDurationWatcher(duration, this);
        }
    }
    ...
    public void cancel() {
        ...
        //让Toast消失
        if(isCustomTime&&toastCustom!=null){
            toastCustom.cancel();
        }else if(toast!=null){
            toast.cancel();
        }
    ...
    public static final class Builder {
         ...
         * @return
         */
        public Builder isCustomTime(boolean isCustomTime){
