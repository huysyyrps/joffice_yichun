...
import com.hy.powerplatform.oa_flow.bean.FlowCompMessage;
...
public class FlowCompMessageDetailActivity extends BaseActivity {
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.btnHistory:
                ...
                ProgressDialogUtil.startLoad(FlowCompMessageDetailActivity.this, "获取数据中");
                new Thread(new Runnable() {
                    @Override
                    public void run() {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case 111:
                    ...
                    adapter = new FlowMessageAdapter(FlowCompMessageDetailActivity.this, flowList);
                    recyclerView.setAdapter(adapter);
                    Toast.makeText(FlowCompMessageDetailActivity.this, "查询成功", Toast.LENGTH_SHORT).show();
                    ProgressDialogUtil.stopLoad();
                    break;
                case TAG_ONE:
                    Gson gson = new Gson();
                    FlowCompMessage bean = gson.fromJson(res, FlowCompMessage.class);
                    ...
                    final String fbpt1 = bean.getMainform().get(0).getFbpt1();
                    final String fbpt2 = bean.getMainform().get(0).getFbpt2();
                    final String fbpt3 = bean.getMainform().get(0).getFbpt3();
                    final String bmfzryj = bean.getMainform().get(0).getBmfzryj();
                    final String fgldyj = bean.getMainform().get(0).getFgldyj();
                    final String zhglbyj = bean.getMainform().get(0).getZhglbyj();
                    ...
                    if (!zhglbyj.equals("")) {
                        tvLeader2.setText(getJSONData(zhglbyj));
                    }
                    if (!jbbmyj.equals("")) {
                    ...
                    break;
                case TAG_TWO:
                    Toast.makeText(FlowCompMessageDetailActivity.this, "获取数据失败，请重试", Toast.LENGTH_SHORT).show();
                    ProgressDialogUtil.stopLoad();
                    break;
                case TAG_NINE:
                    Gson gson2 = new Gson();
                    ...
                    break;
                case TAG_FIVE:
                    Toast.makeText(FlowCompMessageDetailActivity.this, msg.getData().getString("msg"), Toast.LENGTH_SHORT).show();
                    ProgressDialogUtil.stopLoad();
                    finish();
                    break;
                case TAG_SIX:
                    Toast.makeText(FlowCompMessageDetailActivity.this, "提交数据失败", Toast.LENGTH_SHORT).show();
                    ProgressDialogUtil.stopLoad();
                    break;
            }
        }
