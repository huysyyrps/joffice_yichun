...
import com.hy.powerplatform.oa_flow.bean.FlowJSGCWillDetail;
...
public class FlowJSGCWillDetailActivity extends BaseActivity {
    ...
    List<FlowJSGCWillDetail.TransBean> beanList = new ArrayList<>();
...
//                res = dbA.OAQingJiaWillDoDex(url);
    ...
    protected int provideContentViewId() {
        return R.layout.activity_jsgc_data;
    }
    ...
    protected void rightClient() {
        ...
        tv_yes.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View arg0) {
                ...
                if (str.equals("")) {
                    Toast.makeText(FlowJSGCWillDetailActivity.this, getResources().getString(R.string.nullify_reason), Toast.LENGTH_SHORT).show();
                } else {
                    dialog.dismiss();
                    ProgressDialogUtil.startLoad(FlowJSGCWillDetailActivity.this, getResources().getString(R.string.get_data));
                    new Thread(new Runnable() {
                        @Override
                        public void run() {
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            ...
            case R.id.btnT:
                ...
                if (beanList.size() != 0) {
                    if (beanList.size() == 1) {
                        ...
                        MyAlertDialog.MyListAlertDialog(FlowJSGCWillDetailActivity.this, namelist, new AlertDialogCallBackP() {
                            @Override
                    ...
                    Toast.makeText(FlowJSGCWillDetailActivity.this, "审批人为空", Toast.LENGTH_SHORT).show();
                }
                break;
            case R.id.tvData:
            ...
            case R.id.btnUp:
                gson = new Gson();
                bean = gson.fromJson(res, FlowJSGCWillDetail.class);
    ...
    private void personSession() {
        if (bigResultList.size() != 0) {
            ...
            if (btnT.getVisibility() == View.VISIBLE) {
                if (btnTTag.equals("N")) {
                    Gson gson = new Gson();
                    FlowJSGCWillDetail bean = gson.fromJson(res, FlowJSGCWillDetail.class);
                    xqbmyj = bean.getMainform().get(0).getBmfzryj();
                    xqbmldyj = bean.getMainform().get(0).getFgldyj();
                    jsbmyj = bean.getMainform().get(0).getJjbzbyj();
                    jsbmldyj = bean.getMainform().get(0).getJbfgldyj();
    ...
    private void sendData() {
        ProgressDialogUtil.startLoad(FlowJSGCWillDetailActivity.this, "提交数据中");
        new Thread(new Runnable() {
            @Override
            public void run() {
                ...
                final String userCode = new SharedPreferencesHelper(FlowJSGCWillDetailActivity.this,
                        "login").getData(FlowJSGCWillDetailActivity.this, "userCode", "");
                if (codetemp != null) {
                ...
                if (bigResultList.size() == 0 && bigResultList1.size() != 0) {
                    Looper.prepare();
                    ProgressDialogUtil.stopLoad();
                    Toast.makeText(FlowJSGCWillDetailActivity.this, "请选择审批人", Toast.LENGTH_SHORT).show();
                    Looper.loop();
                } else {
    ...
    Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case 333:
                    ProgressDialogUtil.stopLoad();
                    Toast.makeText(FlowJSGCWillDetailActivity.this, getResources().getString(R.string.c_success), Toast.LENGTH_SHORT).show();
                    finish();
                    break;
                case 444:
                    ProgressDialogUtil.stopLoad();
                    Toast.makeText(FlowJSGCWillDetailActivity.this, getResources().getString(R.string.c_false), Toast.LENGTH_SHORT).show();
                    break;
                case 111:
                    Gson gsonF = new Gson();
                ...
                case TAG_ONE:
                    ...
                    final String date = bean.getMainform().get(0).getRiQi();
                    final String person = bean.getMainform().get(0).getShenQingRen();
                    final String department = bean.getMainform().get(0).getXuQiuBuMen();
                    ...
                    if (beanList.size() == 1) {
                        ...
                        ProgressDialogUtil.startLoad(FlowJSGCWillDetailActivity.this, "获取审核人");
                        getAppRovePerson();
                    }
                    break;
                case TAG_TWO:
                    Toast.makeText(FlowJSGCWillDetailActivity.this, "操作数据失败", Toast.LENGTH_SHORT).show();
                    ProgressDialogUtil.stopLoad();
                    break;
                case TAG_THERE:
                    Toast.makeText(FlowJSGCWillDetailActivity.this, "提交数据成功", Toast.LENGTH_SHORT).show();
                    ProgressDialogUtil.stopLoad();
                    finish();
                    break;
                case TAG_FOUR:
                ...
                case TAG_SEVEN:
                    ...
                    break;
                case TAG_EIGHT:
                    Toast.makeText(FlowJSGCWillDetailActivity.this, "流程已到最后一步，请提交", Toast.LENGTH_SHORT).show();
                    ProgressDialogUtil.stopLoad();
                    break;
                case TAG_NINE:
                    Gson gson2 = new Gson();
