    ...
    protected void rightClient() {
        ...
        tv_yes.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View arg0) {
                ...
                if (str.equals("")) {
                    Toast.makeText(FlowWorkPuechaseWillDetailActivity.this, getResources().getString(R.string.nullify_reason), Toast.LENGTH_SHORT).show();
                } else {
                    dialog.dismiss();
                    ProgressDialogUtil.startLoad(FlowWorkPuechaseWillDetailActivity.this, getResources().getString(R.string.get_data));
...
//                res = dbA.OAQingJiaWillDoDex(url);
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            ...
            case R.id.btnUp:
                gson = new Gson();
                bean = gson.fromJson(res, FlowWorkPuchase.class);
            ...
            public void run() {
                ...
                if (rb6.isChecked()) {
                    if (resultList.size() >= 6) {
                        resultList1.add(resultList.get(5));
                    }
                }
                if (rb5.isChecked()) {
                    if (resultList.size() >= 5) {
                        resultList1.add(resultList.get(4));
                    }
                }
                if (rb4.isChecked()) {
                    if (resultList.size() >= 4) {
                        resultList1.add(resultList.get(3));
                    }
                }
                if (rb3.isChecked()) {
                    if (resultList.size() >= 3) {
                        resultList1.add(resultList.get(2));
                    }
                }
                if (rb2.isChecked()) {
                    if (resultList.size() >= 2) {
                        resultList1.add(resultList.get(1));
                    }
                }
                if (rb1.isChecked()) {
                    if (resultList.size() >= 1) {
                        resultList1.add(resultList.get(0));
                    }
                }
                if (cb12.isChecked()) {
                    if (bigResultList.size() >= 12) {
                ...
                if (bigResultList.size() == 0 && bigResultList1.size() != 0) {
                    ...
                    Looper.loop();
                }else if (resultList1.size() == 0 && resultList.size() != 0){
                    Looper.prepare();
                    ProgressDialogUtil.stopLoad();
                    Toast.makeText(FlowWorkPuechaseWillDetailActivity.this, "请选择审批人", Toast.LENGTH_SHORT).show();
                    Looper.loop();
    ...
    Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case 333:
                    ProgressDialogUtil.stopLoad();
                    Toast.makeText(FlowWorkPuechaseWillDetailActivity.this, getResources().getString(R.string.c_success), Toast.LENGTH_SHORT).show();
                    finish();
                    break;
                case 444:
                    ProgressDialogUtil.stopLoad();
                    Toast.makeText(FlowWorkPuechaseWillDetailActivity.this, getResources().getString(R.string.c_false), Toast.LENGTH_SHORT).show();
                    break;
                case 111:
                    Gson gsonF = new Gson();
                ...
                case TAG_ONE:
                    ...
                    try {
                        ...
                        if (bmreout.equals("1") && fgreout.equals("1") && zjlreout.equals("1")) {
                            Toast.makeText(FlowWorkPuechaseWillDetailActivity.this, "您对当前流程只有读取权限", Toast.LENGTH_SHORT).show();
                        }
                    } catch (JSONException e) {
                        e.printStackTrace();
                    }
                    ...
                    if (bean.isRevoke()) {
                        Toast.makeText(FlowWorkPuechaseWillDetailActivity.this, "当前流程已被追回", Toast.LENGTH_SHORT).show();
                    }
                    ProgressDialogUtil.stopLoad();
                    if (beanList.size() == 1) {
                ...
                case TAG_SEVEN:
                    ...
                    break;
                case TAG_EIGHT:
                    Toast.makeText(FlowWorkPuechaseWillDetailActivity.this, "流程已到最后一步，请提交", Toast.LENGTH_SHORT).show();
                    ProgressDialogUtil.stopLoad();
                    break;
                case TAG_NINE:
                    Gson gson2 = new Gson();
