...
import com.hy.powerplatform.oa_flow.activity.FlowEMainatinWillDetailActivity;
...
//        }).start();
    ...
    public void run() {
        String url = Constant.BASE_URL2 + Constant.WILLDOLIST + start +
                "&limit=" + limit+"&Q_subject_S_LK="+Constant.EMAINTAINNAME;
        DBHandler dbA = new DBHandler();
        res = dbA.OAQingJiaWillDo(url);
        if (res.equals("获取数据失败") || res.equals("")) {
            handler.sendEmptyMessage(TAG_TWO);
    ...
    public void getAdapterPosition(int position) {
        Intent intent = new Intent(getActivity(), FlowEMainatinWillDetailActivity.class);
