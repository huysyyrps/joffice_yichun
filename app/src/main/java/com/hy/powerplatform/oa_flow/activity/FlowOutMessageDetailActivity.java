...
import com.hy.powerplatform.oa_flow.bean.MyOutMessage;
...
public class FlowOutMessageDetailActivity extends BaseActivity {
    ...
    protected int provideContentViewId() {
        return R.layout.activity_outmessage_data;
    }
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.btnHistory:
                ...
                ProgressDialogUtil.startLoad(FlowOutMessageDetailActivity.this, "获取数据中");
                new Thread(new Runnable() {
                    @Override
                    public void run() {
    ...
    private Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case TAG_ONE:
                    Gson gson = new Gson();
                    MyOutMessage bean = gson.fromJson(res, MyOutMessage.class);
                    ...
                    String heGao = bean.getMainform().get(0).getHeGaoRen();
                    String num = bean.getMainform().get(0).getYinFaFenShu();
                    String wenHao = bean.getMainform().get(0).getFaWenWenHao();
                    String riQi = bean.getMainform().get(0).getWenHaoRiQi();
                    String date = bean.getMainform().get(0).getYinFaRiQi();
                    String xuHao = bean.getMainform().get(0).getFaWenXuHao();
                    ...
                    break;
                case TAG_TWO:
                    Toast.makeText(FlowOutMessageDetailActivity.this, "获取数据失败，请重试", Toast.LENGTH_SHORT).show();
                    ProgressDialogUtil.stopLoad();
                    break;
                case TAG_NINE:
                    Gson gson2 = new Gson();
                    ...
                    break;
                case TAG_FIVE:
                    Toast.makeText(FlowOutMessageDetailActivity.this, msg.getData().getString("msg"), Toast.LENGTH_SHORT).show();
                    ProgressDialogUtil.stopLoad();
                    finish();
                    break;
                case TAG_SIX:
                    Toast.makeText(FlowOutMessageDetailActivity.this, "提交数据失败", Toast.LENGTH_SHORT).show();
                    ProgressDialogUtil.stopLoad();
                    break;
            }
        }
