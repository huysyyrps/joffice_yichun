...
public class FragmentNewGoodsPurchaseList extends Fragment implements FlowContractPayListAdapter.OnGetAdapterPositionListener {
    ...
    private void getData(final int start, final int limit) {
        new Thread(new Runnable() {
            @Override
            public void run() {
                String url = Constant.BASE_URL2 + Constant.MYLIST+start+"&limit="+limit+"&Q_processName_S_EQ="+Constant.NEWGOODPUECHASENAME;
    ...
    public void getAdapterPosition(int position) {
        ...
        Intent intent = new Intent(getActivity(), FlowPurchaseDetailActivity.class);
