...
import com.hy.powerplatform.oa_flow.activity.FlowContractPayDetailActivity;
...
public class FragmentContractPayList extends Fragment implements FlowContractPayListAdapter.OnGetAdapterPositionListener {
    ...
    private void getData(final int start, final int limit) {
        new Thread(new Runnable() {
            @Override
            public void run() {
                String url = Constant.BASE_URL2 + Constant.MYLIST+start+"&limit="+limit+"&Q_subject_S_LK="+Constant.CONTRACEPAYNAME;
    ...
    public void getAdapterPosition(int position) {
        ...
        Intent intent = new Intent(getActivity(), FlowContractPayDetailActivity.class);
