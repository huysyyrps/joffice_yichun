...
import com.hy.powerplatform.oa_flow.activity.FlowSaferllDetailActivity;
...
public class FragmentSaferList extends Fragment implements FlowContractPayListAdapter.OnGetAdapterPositionListener,Runnable {
    ...
    public void run() {
        ...
        if (tag.equals("1")){
            url = Constant.BASE_URL2 + Constant.MYLIST+start+"&limit="+limit
                    +"&Q_subject_S_LK="+Constant.SAFERNAMES;
        }else {
            url = Constant.BASE_URL2 + Constant.MYLIST+start+"&limit="+limit
                    +"&Q_subject_S_LK="+Constant.SAFERNAMEY;
        }
    ...
    public void getAdapterPosition(int position) {
        ...
        Intent intent = new Intent(getActivity(), FlowSaferllDetailActivity.class);
