...
import com.hy.powerplatform.oa_flow.bean.MyLeaveDetail;
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.btnHistory:
                ...
                ProgressDialogUtil.startLoad(FlowLeaveDetailActivity.this, "获取数据中");
                new Thread(new Runnable() {
                    @Override
                    public void run() {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case 111:
                    ...
                    adapter = new FlowMessageAdapter(FlowLeaveDetailActivity.this, flowList);
                    recyclerView.setAdapter(adapter);
                    Toast.makeText(FlowLeaveDetailActivity.this, "查询成功", Toast.LENGTH_SHORT).show();
                    ProgressDialogUtil.stopLoad();
                    break;
                case TAG_ONE:
                    Gson gson = new Gson();
                    MyLeaveDetail bean = gson.fromJson(res, MyLeaveDetail.class);
                    String person = bean.getMainform().get(0).getFullname();
                    String time = bean.getMainform().get(0).getTianDanRiQi();
                    String type = bean.getMainform().get(0).getVocationType();
                    String reason = bean.getMainform().get(0).getShiyou();
                    String starTime = bean.getMainform().get(0).getBeginDate() + " " + bean.getMainform().get(0).getKsdayType();
                    String endTime = bean.getMainform().get(0).getEndDate() + " " + bean.getMainform().get(0).getJsdayType();
                    ...
                    break;
                case TAG_TWO:
                    Toast.makeText(FlowLeaveDetailActivity.this, "获取数据失败，请重试", Toast.LENGTH_SHORT).show();
                    ProgressDialogUtil.stopLoad();
                    break;
                case TAG_NINE:
                    Gson gson2 = new Gson();
                    ...
                    break;
                case TAG_FIVE:
                    Toast.makeText(FlowLeaveDetailActivity.this, msg.getData().getString("msg"), Toast.LENGTH_SHORT).show();
                    ProgressDialogUtil.stopLoad();
                    finish();
                    break;
                case TAG_SIX:
                    Toast.makeText(FlowLeaveDetailActivity.this, "提交数据失败", Toast.LENGTH_SHORT).show();
                    ProgressDialogUtil.stopLoad();
                    break;
            }
        }
