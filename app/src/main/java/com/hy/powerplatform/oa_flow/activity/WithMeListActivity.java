        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                String url = Constant.BASE_URL2 + Constant.WithMeLIST + start + "&limit=" + limit
                        + "&proTypeId=" + tagType+"&beg="+startTine
                        +"&end="+endTime+"&subject="+title;
//                String url = Constant.BASE_URL2 + Constant.WithMeLIST + start + "&limit=" + limit
//                        + "&proTypeId=" + tagType;
                    ...
                    try {
                        ...
                        if (jsonArray.length() == 0 && beanList.size() == 0) {
                            if (recyclerView != null) {
                                Toast.makeText(WithMeListActivity.this, "暂无该时间段内数据.", Toast.LENGTH_SHORT).show();
                            }
                            adapter.notifyDataSetChanged();
        ...
        customDatePicker1 = new CustomDatePickerDay(this, new CustomDatePickerDay.ResultHandler() {
            ...
            public void handle(String time) { // 回调接口，获得选中的时间
                if (sendHttpContent(time.split(" ")[0])){
                    ...
                    Toast.makeText(WithMeListActivity.this, "请选择正确时间", Toast.LENGTH_SHORT).show();
                }
            }
