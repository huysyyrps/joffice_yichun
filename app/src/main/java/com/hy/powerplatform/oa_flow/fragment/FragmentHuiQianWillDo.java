...
import com.hy.powerplatform.oa_flow.activity.FlowHuiQianWillDetailActivity;
...
public class FragmentHuiQianWillDo extends Fragment implements FlowQingJiaWillDoAdapter.OnGetAdapterPositionListener,Runnable {
...
//                        "&limit=" + limit+"&subject="+Constant.EMAINTAINNAME;
...
//        }).start();
    ...
    public void run() {
        String url = Constant.BASE_URL2 + Constant.WILLDOLIST + start +
                "&limit=" + limit+"&Q_subject_S_LK="+Constant.HUIQIANNAME;
        DBHandler dbA = new DBHandler();
        res = dbA.OAQingJiaWillDo(url);
        if (res.equals("获取数据失败") || res.equals("")) {
            handler.sendEmptyMessage(TAG_TWO);
    ...
    public void getAdapterPosition(int position) {
        Intent intent = new Intent(getActivity(), FlowHuiQianWillDetailActivity.class);
